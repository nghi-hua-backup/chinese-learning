...
| 2026-06-16 | QA (Claude) | fix 2.2.1: Last-card Wrong → "Kiểm tra" button hidden — Root cause: `WritingInput` key was `card.id`; re-queued Wrong card has same ID → component not remounted → `submitted=true` persists → button hidden. Fix: `attempt` counter in `VocabSession` increments on `markWrong()`; key changed to `${card.id}-${attempt}` → fresh remount every time. P1 (build ✓ — TypeScript clean, 4 routes). Added BLOCKER item 83 (last card Wrong → Kiểm tra still visible). Items 83 requires manual verification on live site. | PASS (static + code checks). Manual: verify item 83 on live iPad Safari after deploy. |
