...
| 5 | Luyện viết — write correct answer | Green ✓ "Chính xác!" banner; pinyin (gray) + correct Chinese character (text-4xl bold) shown in answer area; "Tiếp →" button appears; card does NOT auto-advance (FR-14) |
| 6 | Luyện viết — write wrong answer | Red ✗ "Sai rồi" banner; pinyin (gray) + correct Chinese character (text-4xl bold) shown in answer area; "Tiếp →" button appears; card re-queues at end only after tapping "Tiếp →" (FR-14) |
...
| # | Flow | Pass condition |
|---|---|---|
| 84 | File opens locally | `chinese-learning/grammar-reference.html` opens in any browser without errors; page title "Ngữ pháp — Tham khảo" and Phồn thể ⇄ Giản thể button visible in header |
| 85 | All 10 Nhóm visible | Page shows 10 Nhóm section headers (Nhóm 1 through Nhóm 10); each has a purple left-border accent and group name |
| 86 | All 51 pattern cards rendered | Scrolling through the page shows exactly 51 pattern cards; each card has a formula row (colored chips), left panel (key char), and example rows |
| 87 | Giản thể default | On page load, Chinese text shows simplified characters; traditional spans hidden (e.g. 你好 not 你好/妳好) |
| 88 | Toggle to Phồn thể | Clicking the toggle button switches all Chinese text to traditional characters (e.g. 來 instead of 来); button label updates |
| 89 | Toggle back to Giản thể | Clicking the toggle again switches back to simplified; traditional spans hidden again |
| 90 | Generator script runs cleanly | `node scripts/generate-grammar-html.js` completes with exit code 0 and prints "Wrote … patterns across … groups"; output file timestamp updates |
| 91 | Auto-sync in /kb-update | After running a `/kb-update` or `/lesson` workflow, Bước 8 regenerates `grammar-reference.html` and adds it to the commit automatically |

### Luyện viết UX — Hide Pinyin + Manual Advance (FR-14)

| # | Flow | Pass condition |
|---|---|---|
| 92 | Question phase — no pinyin | In Luyện viết, before tapping "Kiểm tra": only Vietnamese meaning, optional Hán Việt note, and the textarea writing pad visible — no pinyin text anywhere on screen |
| 93 | Answer area — pinyin after correct | Tap "Kiểm tra" with correct input → green ✓ "Chính xác!" banner; gray pinyin text appears above correct Chinese character (text-4xl bold) in the answer area |
| 94 | Answer area — pinyin after wrong | Tap "Kiểm tra" with wrong or empty input → red ✗ "Sai rồi" banner; same gray pinyin + correct Chinese character (text-4xl bold) appears in the answer area |
| 95 | "Tiếp →" appears; no auto-advance | After "Kiểm tra", "Tiếp →" indigo button appears below the answer area; card stays visible with no automatic transition |
| 96 | "Tiếp →" after correct → Right | Tap "Tiếp →" after correct "Kiểm tra" → card resolved (FSRS called), next card loads (or completion toast if last card) |
| 97 | "Tiếp →" after wrong → re-queue | Tap "Tiếp →" after wrong "Kiểm tra" → current card re-queues at end; next remaining card loads; re-queued card appears again later in same session |
| 98 | Ôn tập — identical behavior | Start an Ôn tập session (/on-tap) in Luyện viết mode: pinyin absent in question, pinyin + Chinese shown in answer, "Tiếp →" button controls advance — identical to Từ vựng session |
...
| 74 | Empty input → auto-Wrong | Tap "Kiểm tra" with blank textarea → red ✗ banner + pinyin + correct character reveal appears; "Tiếp →" button appears; tapping "Tiếp →" re-queues card at end (FR-14) |
| 75 | Correct input → Right on "Tiếp →" | Type the correct character(s), tap "Kiểm tra" → green ✓ "Chính xác!" banner + pinyin + correct character; tap "Tiếp →" → card resolved, next card (or completion toast) |
| 76 | Wrong input → Wrong on "Tiếp →" | Type incorrect text, tap "Kiểm tra" → red ✗ banner + pinyin + correct character reveal; tap "Tiếp →" → card re-queued at end |
...
| 2026-06-19 | QA (Claude) | FR-13 (2.3.0): Grammar Reference HTML — P1 (N/A — standalone HTML, not part of Next.js build), P2 (pushed to origin/main ✓ — commit 1fe9579), P3 (N/A — local file, not deployed URL). Static verification: file exists at `chinese-learning/grammar-reference.html` (112KB) ✓; 51 `.pattern-card` elements ✓; 10 Nhóm groups (Nhóm 1–10) ✓; 51 `.left-panel` + `.key-char` + `.formula-row` ✓; 43 `.formula-note` (amber explanation — some patterns omit) ✓; 171 `.example-row` + `.chinese` + `.pinyin` + `.translation` ✓; 171 `.simp` + 107 `.trad` spans (toggle targets) ✓; 6 toggle-related JS occurrences ✓; SVGs drawn dynamically at runtime (0 static — correct) ✓; generator uses only built-in `fs`+`path` (no npm) ✓; Bước 8 of kb-update.md calls `node scripts/generate-grammar-html.js` + `git add grammar-reference.html` ✓. All 6 ACs verified: AC-1 (file opens locally) ✓; AC-2 (10 Nhóm groups) ✓; AC-3 (formula header + left panel + examples) ✓; AC-4 (Phồn thể/Giản thể toggle) ✓; AC-5 (generator script, no npm) ✓; AC-6 (auto-invoked in /kb-update Bước 8) ✓. No regressions possible — feature is entirely outside Next.js app. Added test scenarios 84–91. Items 84–89 require manual browser verification (open file locally). | PASS (static + code checks). Manual: open `chinese-learning/grammar-reference.html` locally; verify items 84–91. |
| 2026-06-23 | QA (Claude) | FR-14 (2.4.0): Luyện viết UX — hide pinyin in question + manual advance — P1 (build ✓ — commit c6cb7a7, TypeScript clean, compiled in 1929ms). P2 (GitHub Actions — gh CLI unavailable; inferred from c6cb7a7 pushed to origin/main). P3 (live URL ✓ — "Đang tải..." Suspense fallback, expected behavior). Code-level AC verification (all 6 ACs): AC-1 — VocabSession.tsx question card block (lines 110–118) shows only `card.meaning` + `card.hanViet`; no `card.pinyin` rendered; WritingInput.tsx pinyin render gated on `submitted` (line 85) ✓; AC-2 — WritingInput.tsx lines 77–84: green ✓/"Chính xác!" + red ✗/"Sai rồi" banners unchanged ✓; AC-3 — pinyin (line 85) and `{expected}` (line 86) rendered outside correct/wrong conditional → both correct and wrong cases show pinyin + Chinese ✓; AC-4 — handleResult luyen-viet path (lines 89–91) has no setTimeout; "Tiếp →" button at lines 148–155 gated on `answered` ✓; AC-5 — onClick (lines 149–151) dispatches markRight() or markWrong() per lastCorrect ✓; AC-6 — VocabSession shared by /tu-vung and /on-tap; mode-gated code applies equally ✓. Updated stale BLOCKER items 5, 6, 74, 75, 76 (removed "auto-advances after ~1.5 s" references). Added test scenarios 92–98. Items 92–98 require JS execution in live browser. | PASS (static + code checks). Manual: verify items 92–98 on live iPad Safari. |
