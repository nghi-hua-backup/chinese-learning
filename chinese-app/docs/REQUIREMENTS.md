...
### PF-2: Grammar Reference HTML
**Priority:** High
**Description:** Standalone `chinese-learning/grammar-reference.html` — opened locally in a browser, never part of the Next.js app. Shows all grammar patterns from §5 Ngữ pháp of `chinese-brain.md`, grouped by Nhóm, in a layout inspired by the "100 Chinese Grammar Formulas" book. Generated by `scripts/generate-grammar-html.js` (Node.js, no external dependencies), auto-invoked at the end of every `/lesson` and `/kb-update` run.
**Acceptance criteria:**
...
- AC-3: Each pattern card has a formula header (large colored components + purple Vietnamese subtitle + amber explanation note), left panel (pinyin → 56px red key char → meaning word), fan SVG arrows, and example rows (pinyin / 36px colored Chinese / Vietnamese translation)
